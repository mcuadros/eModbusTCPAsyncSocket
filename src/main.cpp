...
IPAddress ip = {10, 0, 0, 11};          // IP address of modbus server
...
// Set up ModbusTCP client.  if (!WiFi.config(ip, _gateway, _subnet)) {
...
void loop() {
  ...
  if (millis() - lastMillis > 5000) {
    ...
    Error err;
    err = MB.addRequest((uint32_t)lastMillis, 1, READ_HOLD_REGISTER, 1, 2);
    if (err != SUCCESS) {
      ModbusError e(err);
